...
from gen_ai_hub.proxy.core import get_proxy_version, set_proxy_version
...
bedrock = Session().client(model_name="anthropic--claude-3-sonnet")


class AICoreProxyOpenAIClient(OpenAIClient):
    def __init__(self, config, client: Optional[OpenAI] = None):
        """
        SAP's AI Core OpenAI proxy client.
        :param config: Configuration for the client. Not currently used.
        :param client: OpenAI proxy client to use. If None, a new client will be created.
        """
        if client is None:
            client = OpenAIProxy()
        # Suppress warnings about malformatted OpenAI API key.
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            super().__init__(client)
    
    def create(self, params: Dict[str, Any]) -> ChatCompletion:
        # Remove unneeded params before passing to the parent class method
...
config_list = [
    # {"model": "claude-3-5-sonnet", "model_client_cls": "AICoreProxyOpenAIClient", "model_purpose": "agent"},
    {"model": "anthropic--claude-3-5-sonnet", "model_client_cls": "AICoreProxyOpenAIClient", "model_purpose": "manager"}
]
...
async def plan_project(user_input, iterations):
    ...
    here is the user input: {user_input}
    """}]
    messages_1 = [{"role": "user", "content": [{"text":f"please plan a Pygame project based on the following user input: {user_input}. Remember that the game should start with a main module in the main.py file."}]}]
    messages_2 = []
    ...
    for i in range(iterations):
        ...
        if PRINT_RESPONSE:
            print(colored(response_2['output']['message']['content'][0]['text'], "blue"))
        messages_2.append({"role": "assistant", "content": [{"text":response_2['output']['message']['content'][0]['text']}]})
        messages_1.append({"role": "user", "content": [{"text": response_2['output']['message']['content'][0]['text']}]})
    
    # Extract the XML content from the response
    xml_content = re.search(r'<game_plan>.*?</game_plan>', response_2['output']['message']['content'][0]['text'], re.DOTALL)
    if xml_content:
        return xml_content.group(0)
    else:
        raise ValueError("No valid XML content found in the response")
    
async def write_file_async(file_name, code):
    """Asynchronously writes the file to disk using asyncio.to_thread."""
    def write_file():
    ...
    print(f"File '{file_name}' has been written to disk.")
...
async def agent_write_file(file_name, file_description, game_plan):
    ...
    await write_file_async(file_name, code)
...
async def update_game_files(user_feedback):
    ...
    system_message = [{"text":"""You are an expert Python and Pygame developer. Your task is to update a Pygame project based on user feedback. 
    ...
    if PRINT_RESPONSE:
        print(colored(response['output']['message']['content'][0]['text'], "magenta"))

    # Extract updated file contents from the response
    updated_files = re.findall(r'<file name="(.*?)">(.*?)</file>', response['output']['message']['content'][0]['text'], re.DOTALL)
...
async def fix_game_files(error_message):
    ...
    corrected_files = re.findall(r'<file name="(.*?)">(.*?)</file>', response['output']['message']['content'][0]['text'], re.DOTALL)
